...
func (sc *ServiceRouteController) ServiceRoute(rg *gin.RouterGroup) {
	...
	router.POST("/user-service/history", middleware.DeserializeUser(), sc.serviceController.AddServiceHistory)
	router.GET("/user-service/history", middleware.DeserializeUser(), sc.serviceController.GetServiceHistories)
